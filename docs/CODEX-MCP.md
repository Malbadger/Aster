...
LAW exposes a local MCP server that lets Codex delegate bounded read-only work to Pi and
...
- `law_ollama_start_readonly` — start bounded LAW/Pi inspection and return immediately.
...
attended `law run --mutate` workflow, where the existing LAW policy gates remain visible.
...
3. “Export the evidence for the LAW run you just completed.”

Only the bounded result returned by a tool enters the Codex conversation. Pi's full
session transcript, LAW checkpoints, model weights, and evidence remain on this device.
...
Codex and Claude Code share the selected model and LAW evidence. Background jobs live in
the client-owned MCP process; restarting that client interrupts an active job, while
completed LAW checkpoints and evidence remain on disk.
